...
define BUSYBOX_INSTALL_CROND_SCRIPT
	if grep -q CONFIG_CROND=y $(@D)/.config; then \
		[ -f $(TARGET_DIR)/etc/init.d/S60crond ] || \
		$(INSTALL) -m 0755 -D package/busybox/S60crond \
		$(TARGET_DIR)/etc/init.d/S60crond; \
		mkdir -p $(TARGET_DIR)/etc/cron/crontabs/; \
		cp package/busybox/root $(TARGET_DIR)/etc/cron/crontabs/root; \
	else rm -fr $(TARGET_DIR)/etc/init.d/S60crond $(TARGET_DIR)/etc/cron; fi
endef
